...
class ApriltagR(Node):
    def __init__(self):
        super().__init__("apriltag_r_output")    # names the node when running
        self.kp = 0.5
        self.ki = 0.01
        self.kd = 0.1
        ...
        self.goal_sub = self.create_subscription(
            PoseArray, "/tags", self.r_control, 10)

        self.pub = self.create_publisher(Float64, "/ApriltagR_control_output", 10)

        self.get_logger().info("initialized ApriltagR node")
